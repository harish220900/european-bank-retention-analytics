...
def load_data():
    ...
    paths = [
        'European_Bank.csv',
        os.path.join(os.getcwd(), 'European_Bank.csv'),
        '/mount/src/european-bank-retention-analytics/European_Bank.csv',
    ]
    for p in paths:
        try:
            df = pd.read_csv(p)
            ...
            continue
    if df is None:
        raise FileNotFoundError("European_Bank.csv not found")
...
try:
    ...
    st.error("Please ensure 'European_Bank.csv' is in the same directory as this dashboard.")
    st.stop()
...
with tab5:
    ...
    with col1:
        ...
        fig.add_trace(go.Bar(
            ...
            marker_color=px.colors.sequential.RdYlGn[::2][:len(rsi_data)],
